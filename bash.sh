...
command=$(ls|grep 'alpha_beta_player\|my_player')
...
java=$([[ $command =~ (^|[[:space:]])"alpha_beta_player.java"($|[[:space:]]) ]] && echo 'yes' || echo 'no')
if [ "$py" == "yes" ]; then
    ...
    javac alpha_beta_player.java
	cmd="java alpha_beta_player"
	echo "JAVA"
elif [ "$c11" == "yes" ]; then
...
prefix="/mnt/d/USC/Coursework/Fall 2021/CS 561 - Artificial Intelligence/Homework-2/Go"
